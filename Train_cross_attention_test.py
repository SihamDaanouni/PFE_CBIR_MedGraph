...
def run_cross_attention_pipeline(csv_filename: str, img_dirname: str):
    ...
    print("\n[CHECK] Dataset split verification")
    ...
    t_X_img_train = torch.tensor(X_img_train, dtype=torch.float32).to(device)
    t_X_txt_train = torch.tensor(X_txt_train, dtype=torch.float32).to(device)
    t_Y_train = torch.tensor(Y_train, dtype=torch.long).to(device)

    print(f"\nTraining on {len(Y_train)} images. Testing on {len(Y_test)} images.")

    fusion_module = CrossAttentionFusion(embed_dim=512).to(device)
    classifier_head = nn.Linear(512, 8).to(device)

    optimizer = optim.Adam(
        list(fusion_module.parameters()) + list(classifier_head.parameters()),
        lr=1e-3,
    )
    ...
    t_X_img_test = torch.tensor(X_img_test, dtype=torch.float32).to(device)
    t_X_txt_test = torch.tensor(X_txt_test, dtype=torch.float32).to(device)
    ...
    print("\n[CHECK] Evaluation verification")
    print("Final test embeddings shape:", final_test_embeddings.shape)
    ...
    total_relevant_by_class = {
        int(label): int(count - 1)
        for label, count in zip(unique_test_labels, test_counts)
    }

    print("Total relevant per class in TEST SET:", total_relevant_by_class)

    evaluator = RetrievalEvaluator(k_values=[5, 10, 20, 30, 100])
    mean_p, mean_r, mean_map, mean_f1 = evaluator.compute_metrics(
        similarity_matrix,
        Y_test,
    )
    ...
    for k in [10, 20]:
        print(
            f"P@{k}: {mean_p[k]:.4f} | "
            f"MAP@{k}: {mean_map[k]:.4f} | "
            f"F1@{k}: {mean_f1[k]:.4f}"
        )


if __name__ == "__main__":
    run_cross_attention_pipeline(
        "isic_balanced_multimodal.csv",
        "isic_balanced_images",
    )